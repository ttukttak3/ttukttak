...
const UpdateBookInfoPage = ({ categoryList, bookInfo, item }) => {
  ...
  const [bookTitle, setBookTitle] = useState(name);
  ...
  const bookThumbnail = bookInfo.thumbnail.imageUrl;
  ...
  // 콤마
  const chgPrice = price.toString().replace(/\B(?=(\d{3})+(?!\d))/g, ',');

  const { Wrapper, ContentsBox, ImageContainer, UplodImgBox, OptionText, ImgBox, SaveButton, VerticalScrollWrapper, Text } = style;
  ...
  return (
    <Wrapper>
            ...
            ? imgPreview.map((preview, index) => {
                return (
                  <UplodImgBox key={index}>
                    ...
                    <button className={preview.imageUrl === bookThumbnail ? 'hide' : ''} name={preview.imageUrl ? preview.imageUrl : preview} onClick={onClickImageRemove}>
                      x
                    </button>
                  </UplodImgBox>
                );
      ...
      <ContentsBox>
        ...
        <input placeholder="저자명" value={bookAuthor} onChange={e => setBookAuthor(e.target.value)} disabled />
        <Text>
          도서 정가<p>₩{chgPrice}</p>
        </Text>
